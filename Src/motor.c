...
uint16_t rightcalPWMCount(float wvel_r) {
	...
	if ((fabs(g_targetTrans.wvel_r) > 0.0)
			&& (6534512.719 / fabs(g_targetTrans.wvel_r) < UINT16_MAX)) {
		PWMCount = (uint16_t) (6534512.719 / fabs(g_targetTrans.wvel_r) ) - 1;
	} else {
...
uint16_t leftcalPWMCount(float wvel_l) {
	...
	if ((fabs(g_targetTrans.wvel_l) > 0.0)
			&& (6534512.719 / fabs(g_targetTrans.wvel_l) < UINT16_MAX)) {
		PWMCount = (uint16_t) (6534512.719 / fabs(g_targetTrans.wvel_l) ) - 1;
	} else {
		PWMCount = UINT16_MAX - 1;
	}
	return PWMCount;
...
void rightcalPara(tarparameter *para) {
	para->dis += para->wvel_r * DT + para->acc * DT * DT / 2.0;
}//SEKIBUNKANSUU_MIGI

void leftcalPara(tarparameter *para) {
	para->dis += para->wvel_l * DT + para->acc * DT * DT / 2.0;
